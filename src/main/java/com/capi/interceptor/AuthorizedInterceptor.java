...
public class AuthorizedInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		// 设置不拦截的对象
		String[] noFilters = new String[] { "login", "signUp","logOut" };
		...
		if (beFilter == true) {// 除了不拦截的对象以外
			 ...
			 * 1、用户选择记住密码：取出cookie中的用户名和密码查询,如果此用户已不存在，则清除cookie中的值，如果存在则判断用户是否重新登录,
			 * 如果未重新登录则将cookie中用户信息设置到session中,如果用户重新登录了则判断登录用户是否与cookie中用户一致，
			 * 一致则将cookie中用户信息设置到session中,不一致则将当前登录用户的信息设置到session中。
			 * 将用户信息放入session中是为了(通过cookie中的用户名密码可以得到用户信息): 1、重开浏览器的时候如果已经登录的用户可以直接进入
			...
			if (cokLoginName != null && cokLoginPwd != null && cokLoginName.getValue() != ""
				...
				if (!userService.checkUser(loginName, loginPwd)) {
				...
				//认证通过
					...
					if (username == null) {// 如果未登录而直接拷贝地址栏进入页面
						request.getSession().setAttribute("username", loginName);
					} else {// 用户登录后
						if (username.equals(loginName)) {// 如果当前登录人与cookie中信息一致
							request.getSession().setAttribute("username", loginName);
