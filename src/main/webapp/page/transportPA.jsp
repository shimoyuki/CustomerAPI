				...
				<div class="row">
					<div class="col-xs-12">
						<div class="box">
							...
							<div class="box-body">
								<!-- 进度条的显示 -->		
								<div class="progress progress-striped active" id="progess">
								...
								<input type="text" class="form-control"
									placeholder="地区/-地区（选择/排除指定地区）" value=""
	...
	<script>
		  $(function() {
			...
			$("#transportPA").click(function(){
				...
				if(confirm('确认发送'+info+number+'条数据？')){
				$.ajax({
					url : 'customer/transportPA',
					type : "POST",
					dataType : "json",
					data :{
						adsId : $("#adsId").val(),
					...
					success : function(data) {
						if (data.result == "success") {
							...
							$(".box-header").append("<span id='tips' style='color:red;font-size:18px'>未知错误，请联系技术人员。</span>");
						}
						$("#customerTable").dataTable().fnDraw(false);
					},
					error : function(res) {
			...
			$("#transpartPA").click(function(){
				...
				if(select != "" && confirm('确认发送id为'+select+'的数据？')){
				$.ajax({
					url : 'customer/transpartPA',
					type : "POST",
					dataType : "json",
					data :{
						adsId : $("#adsId").val(),
				...
				$.ajax({
					...
					success : function(data) {
						...
						if (data.result == "success") {
							...
							if(typeof(data.adsId)!="undefined"){
								$("#statisticsInfo").append("<p>时段内渠道位"+data.adsId+"发送数据成功"+data.successCount+"条，发送数据失败"+data.failCount+"条</p>");
							}
						} else if(data.result == "error") {
							$("#statistics").append("<span id='statisticsInfo' style='color:red;font-size:18px'>无统计信息，起止时间有误或该时段无数据。</span>");
						}else {
							$("#statistics").append("<span id='statisticsInfo' style='color:red;font-size:18px'>登录状态失效，请重新登录。</span>");
						}
					},
					error : function(res) {
						alert(res.responseText);
